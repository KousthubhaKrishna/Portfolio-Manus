import { g as gsapWithCSS, S as Scene, P as PerspectiveCamera, W as WebGLRenderer, B as BufferGeometry, a as BufferAttribute, b as PointsMaterial, A as AdditiveBlending, c as Points, F as Float32BufferAttribute, L as LineBasicMaterial, d as LineSegments } from "./vendor-w6MpGiQ4.js";
(function polyfill() {
 ...
 * @author: Jack Doyle, jack@greensock.com
*/
var gsap, _coreInitted, _window, _docEl, _body, _toArray, _config, ScrollTrigger, _windowExists2 = function _windowExists3() {
  return typeof window !== "undefined";
  ...
  gsap = _getGSAP3();
  if (_windowExists2() && gsap && typeof document !== "undefined" && document.body) {
    _window = window;
    _body = document.body;
...
class PortfolioApp {
  ...
  setupCustomCursor() {
    ...
    interactiveElements.forEach((el) => {
      el.addEventListener("mouseenter", () => {
        gsapWithCSS.to(cursor, { scale: 1.5, duration: 0.3 });
        gsapWithCSS.to(follower, { scale: 1.5, duration: 0.3 });
      });
      el.addEventListener("mouseleave", () => {
        gsapWithCSS.to(cursor, { scale: 1, duration: 0.3 });
        gsapWithCSS.to(follower, { scale: 1, duration: 0.3 });
      });
    });
  }
  // Navigation
  setupNavigation() {
    ...
    ScrollTrigger$1.batch(sections, {
      onEnter: (elements) => {
  ...
  setupHeroBackground() {
    ...
    for (let i = 0; i < particleCount * 3; i += 3) {
      ...
      } while (Math.abs(x) < 50 && Math.abs(y) < 40);
      positions[i] = x;
      positions[i + 1] = y;
      positions[i + 2] = z;
      const intensity = 0.4 + Math.random() * 0.3;
      colors[i] = 0 * intensity;
      colors[i + 1] = 0.83 * intensity;
      colors[i + 2] = 1 * intensity;
    ...
    const animate = (currentTime) => {
      ...
      particleSystem.rotation.x += 3e-4;
      particleSystem.rotation.y += 7e-4;
      lines.rotation.x += 2e-4;
      lines.rotation.y += 5e-4;
  ...
  setupScrollAnimations() {
    gsapWithCSS.utils.toArray(".fade-in").forEach((element) => {
      gsapWithCSS.fromTo(
        element,
        { opacity: 0, y: 50 },
        {
    ...
    gsapWithCSS.utils.toArray(".timeline-item").forEach((item, index) => {
      gsapWithCSS.fromTo(
        item,
    ...
    gsapWithCSS.utils.toArray(".project-card").forEach((card, index) => {
      gsapWithCSS.fromTo(
        card,
    ...
    gsapWithCSS.utils.toArray(".skill-item").forEach((skill, index) => {
      gsapWithCSS.fromTo(
        skill,
        { opacity: 0, scale: 0 },
        {
    ...
    gsapWithCSS.utils.toArray(".section-title").forEach((title) => {
      gsapWithCSS.fromTo(
        title,
        { opacity: 0, y: 50 },
        {
  ...
  setupCounterAnimations() {
    ...
    counters.forEach((counter) => {
      ...
      const suffix = counter.textContent.includes("B") ? "B+" : counter.textContent.includes("K") ? "K+" : counter.textContent.includes("%") ? "%" : "+";
  ...
  setupSmoothScrolling() {
    ...
    gsapWithCSS.utils.toArray(".hero-bg").forEach((bg) => {
      gsapWithCSS.to(bg, {
        yPercent: -50,
        ease: "none",
        scrollTrigger: {
...
window.addEventListener("load", () => {
  gsapWithCSS.to(".hero-title .title-line", {
    opacity: 1,
    y: 0,
    duration: 1,
    stagger: 0.2,
